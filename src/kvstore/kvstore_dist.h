    ...
    if (IsWorkerNode()) {
      ...
      ps_worker_ = new ps::KVWorker<char>(0, new_customer_id);
        ...
        ps_worker_->set_request_handle(
                std::bind(&KVStoreDist::WorkersMerge, this, _1, _2, _3));
...
void WorkersMerge( const ps::KVMeta& req_meta,const ps::KVPairs<char> &req_data, ps::KVWorker<char>* worker) {
    if(req_meta.num_merge==-1){ //it's time to send push request
        ...
        update_buf_[key].merged.WaitToRead(); ps_worker_->Send2(req_meta_buf[key].timestamp, req_meta_buf[key].push, req_meta_buf[key].cmd, req_data_buf[key], req_meta_buf[key].key , req_meta_buf[key].version, req_meta_buf[key].app_id, req_meta_buf[key].customer_id,req_meta_buf[key].num_merge);
...
struct UpdateBuf {
  ...
  //std::unordered_map<int, UpdateBuf> update_buf_;
  //ps::KVMeta req_meta_buf;
  //ps::KVPairs<char> req_data_buf;
  /**
    ...
    for (size_t i = 0; i < uniq_keys.size(); ++i) {
      ...
      auto pull_from_servers = [this, key, recv_buf](
        ...
        if(gradient_compression_->get_type() == CompressionType::kNone){
            data_version_[key] = CHECK_NOTNULL(ps_worker_)->AutoPull(
  ...
  void PushCompressed(int key, const NDArray& comm_buf, const PSKV& pskv, int priority) {
    ...
    auto push_to_servers =
      [this, key, dtype, pskv, small_buf](RunContext rctx, Engine::CallbackOnComplete cb) {
        ...
        CHECK_NOTNULL(ps_worker_)->ZPush(pskv.keys, vals, pskv.lens, cmd, [cb]() { cb(); }, key, 0);
      };
    ...
    auto push_to_servers =
        [this, key, pskv, send_buf](RunContext rctx, Engine::CallbackOnComplete cb) {
          ...
          CHECK_NOTNULL(ps_worker_)->ZPush(
              ...
              cmd, [cb]() { cb(); }, key, data_version_[key]);
        };
    Engine::Get()->PushAsync(
        push_to_servers,
        pinned_ctx_,
