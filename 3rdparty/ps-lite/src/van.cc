...
void Van::Start(int customer_id) {
  ...
  if (init_stage == 0) {
    ...
    if (is_scheduler_) {
      my_node_ = scheduler_;
        max_greed_rate= atof(Environment::Get()->find("MAX_GREED_RATE"));
        PS_VLOG(2)<<"max_greed_rate is : "<<max_greed_rate;
...
//vbc : process autopull reply msg for unlocking pull msg barrier
...
//vbc : ask for scheduler to return receiver_id for pull msg (ask msg)
...
//vbc : ask for scheduler to return receiver_id for push msg (ask1 msg)
...
//vbc : scheduler processes ask1 msg to return a recviver_id
void Van::ProcessAsk1Command(Message* msg){
    ...
    if(ask_q.size()>1){
        ...
        if(node_a==8 ||node_b==8){
            if(node_a==8){
                ...
                B1[node_b]=1;
                Send(rpl);
                //PS_VLOG(2)<<node_b<<" send to 8";
            }else{
                rpl.meta.iters=node_b;
                rpl.meta.recver = node_a;
                B1[node_a]=1;
                Send(rpl);
                //PS_VLOG(2)<<node_a<<" send to 8";
            }
        }else{
            if(A[node_a][node_b]>A[node_b][node_a]){
                ...
                Send(rpl);
                B1[node_a]=1;
                //PS_VLOG(2)<<"sender is "<<node_a<<" and receiver is "<<node_b;
            }else {
                //rpl.meta.customer_id=0;
                //rpl.meta.recver = node_a;
                //Send(rpl);
                ...
                Send(rpl);
                B1[node_b]=1;
                //PS_VLOG(2)<<"sender is "<<node_b<<" and receiver is "<<node_a;
            }
        }
    }
    ...
    if(count==Postoffice::Get()->num_workers()){
        ...
        //PS_VLOG(2)<<"Push is over!";
    }

    lk_sch1.unlock();
}
//vbc : scheduler processes ask msg to return a receiver_id
    void Van::ProcessAskCommand(Message* msg)
    {
        ...
        //PS_VLOG(2)<<"[iters]: "<<msg->meta.version<<", "<<req_node_id<<" => "<<rpl.meta.customer_id;
        lks.unlock();
        Send(rpl);
    }
//vbc : server or worker process ask reply msg to unlock pull barrier
...
//vbc : server or worker wait here for a autopull
