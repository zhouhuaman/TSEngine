...
namespace ps {
    ...
    void Postoffice::InitEnvironment() {
        ...
        std::string role(val);
        //val = CHECK_NOTNULL(Environment::Get()->find("DMLC_NUM_GROUP"));
        //num_groups_ = atoi(val); //add by cqq
    ...
    void Postoffice::Manage(const Message& recv) {
        ...
        if (ctrl.cmd == Control::BARRIER && !recv.meta.request) {
            barrier_mu_.lock();
            for (std::size_t customer_id = 0; customer_id < barrier_done_[recv.meta.app_id].size();
                 customer_id++) {
    ...
    std::vector<int> Postoffice::GetDeadNodes(int t) {
        ...
        if (!van_->IsReady() || t == 0) return dead_nodes; //初始状态，van还没有准备好
