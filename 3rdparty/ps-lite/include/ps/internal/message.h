...
struct Control {
  ...
  std::string DebugString() const {
    ...
    std::vector<std::string> cmds = {
      "EMPTY", "TERMINATE", "ADD_NODE", "BARRIER", "ACK", "HEARTBEAT","AUTOPULLRPY","ASK","REPLY", "ASK1"};
    std::stringstream ss;
    ss << "cmd=" << cmds[cmd];
    if (node.size()) {
      ss << ", node={";
  ...
  /** \brief all commands */
  enum Command { EMPTY, TERMINATE, ADD_NODE, BARRIER, ACK, HEARTBEAT,AUTOPULLRPY,ASK, REPLY, ASK1 };
  /** \brief the command */
  Command cmd;
  /** \brief node infos */
